...
The _sandbox_ _developer portal_ acts as a Certificate Authority (CA) and provides SSL Client Certificates to the TPP developers. The _ssl-proxy_ needs access to the _sandbox_ CA certificate in order to verify the TPP SSL client certificates (see <<Configuration>>).
...
Images are released to https://cloud.docker.com/u/adorsys/repository/docker/adorsys/psd2-sandbox[adorsys DockerHub account].

[options="header"]
|=======
| Artifact  | Image Name
| sandbox   | adorsys/psd2-sandbox:{sandbox-version}
| ssl-proxy | adorsys/psd2-sandbox-ssl-proxy:{sandbox-version}
|=======

If you want to deploy the JAR without Docker please extract it from the Docker image. Use this command to copy the JAR to your current directory.

[subs=attributes+]
[source,shell]
----
$ docker run -it --rm -v $PWD:/target adorsys/psd2-sandbox:{sandbox-version} sh -c "cp /opt/app-root/src/*.jar /target"
----
...
COPY --from=adorsys/psd2-sandbox:{sandbox-version} /opt/app-root/src/sandbox-{sandbox-version}.jar .

CMD java -jar sandbox-{sandbox-version}.jar
----
...
- _default_ for https://en.wikipedia.org/wiki/Privacy-Enhanced_Mail[PEM] format without line breaks
...
|=======
| Property  | Description | Default
| API_INTERNAL_URL    | Internal URL where the XS2A API is reachable (set in docker-compose) | `http://service:8080`
| API_EXTERNAL_URL    | External URL where the XS2A API is reachable (set in docker-compose) | `api.sandbox.vcap.me`
| PORTAL_INTERNAL_URL | Internal URL where the Sandbox is reachable (set in docker-compose)  | `http://service:8081`
| PORTAL_EXTERNAL_URL | External URL where the Sandbox is reachable (set in docker-compose)  | `portal.sandbox.vcap.me`
|=======
...
| /opt/app-root/etc/ca.crt    | Sandbox CA Certificate | default adorsys CA certificate
|=======

NOTE: The Sandbox CA certificate (ca.crt) is already embedded in the _ssl-proxy_. If you supply your own instance of this component you need to configure the proper SSL and CA certificates there. You can get the Sandbox CA certificate by extracting it from the _ssl-proxy_ image. See <<Deploy on Your Own Infrastructure>> for more details.
...
The PSD2 Sandbox deployment is quite flexible. The only required component is the _sandbox_ and its DB.
...
WARNING: As of now we release Docker Images with a Spring Boot fat JAR with an embedded Tomcat only. We do not release the PSD2 Sandbox as a WAR (yet).
...
$ docker run -it --rm -v $PWD:/target adorsys/psd2-sandbox-ssl-proxy:{sandbox-version} sh -c "cp /opt/app-root/etc/ca.crt /target"
----
...
- HA setup is possible, all state is shared in the DB
- supported databases (see <<Use Your Own Database>>):
  ** PostgreSQL 9+
  ** Oracle 11g+
  ** MariaDB 10+
- 2-4 GB RAM per _sandbox_ deployment is a good starting point for prod *(monitor this!)*
- _ssl-proxy_ needs 128 - 512 RAM MiB *(don't use in prod!)*
...
[source,shell]
----
$ docker run -it --rm -v $PWD/ojdbc8.jar:/opt/app-root/src/lib adorsys/psd2-sandbox:{sandbox-version}
----

If you run the PSD2 Sandbox without Docker, you need to set `loader.path` yourself:
...
$ java -jar -Dloader.path="lib" sandbox-{sandbox-version}.jar
----
...
NOTE: Every approach needs at least read access to the DB.
...
The `sandbox-{sandbox-version}.jar` can be used as CLI application to do the migration with different credentials than the actual deployment.

[subs=attributes+,source,shell]
----
$ java -jar sandbox-{sandbox-version}.jar migrate --spring.datasource.url=$JDBC_URL --spring.datasource.username=$USR --spring.datasource.password=$PASS
----
...
[subs=attributes+,source,shell]
----
$ java -jar sandbox-{sandbox-version}.jar create-schema --spring.datasource.url=$JDBC_URL --spring.datasource.username=$USR --spring.datasource.password=$PASS
----

=== Build Pipeline

We use https://git.adorsys.de/psd2/sandbox/pipelines[GitLab CI] for the pipeline. Build, release, testing, Code coverage, linting and compilation/deployment of this architecture documentation are automated. Every artifact we build is also kept for one week and can be downloaded in corresponding pipeline/job.

Every commit on master pushes images for _sandbox_ and _ssl-proxy_ to the adorsys OpenShift. An image change trigger then starts a new deployment. The arc42 documentation is also built on every commit and published with the https://git.adorsys.de/psd2/sandbox/pipelines[GitLab Pages] feature.

Each tag which follows the semver pattern with a leading `v` like `v1.0.0` results in a tagged release. Tags are pushed to https://harbor.adorsys.de (see <<Release Artifacts>>). Releases are created locally with our `infrastructure/build/release.sh` script.

=== Deployment Samples

==== docker-compose

This docker-compose setup will start the _sandbox_ on port `:8080`/`:8081` and the _ssl-proxy_ on port `:8443`. The _ssl-proxy_ knows how to serve the domains https://portal.sandbox.vcap.me and https://api.sandbox.vcap.me locally with a self signed SSL certificate.
...
This is an excerpt of the adorsys OpenShift deployment on https://sandbox-api.dev.adorsys.de/swagger-ui.html and https://sandbox-portal.dev.adorsys.de. The PostgreSQL deployment we use is not shown here.
