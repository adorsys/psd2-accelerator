...
| LOGGING_CONFIG                                 | Location of the logback configuration  | _classpath:sandbox-logback.xml_
...
[source,yaml]
----
include::../../service/src/main/resources/sandbox-logback.xml[]
----

WARNING: If you use your own `logback.xml` we can't guarantee that the default Spring Boot configuration properties still work. Please check the docs thoroughly. Have a look at the https://docs.spring.io/spring-boot/docs/1.5.18.RELEASE/reference/html/howto-logging.html[Spring Boot logging configuration], the https://logback.qos.ch/manual/configuration.html[Logback Documentation] and our `sandbox-logback.xml` to see whats possible.
