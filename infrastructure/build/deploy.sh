...
if [ "$1" == "latest" ]; then
  ...
  docker tag adorsys/$SSL_PROXY_IMAGE:latest $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_SANDBOX_INTEG/$SSL_PROXY_IMAGE:latest
  docker push $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_SANDBOX_INTEG/$SSL_PROXY_IMAGE:latest
