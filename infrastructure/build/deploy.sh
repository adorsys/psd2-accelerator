...
if [ "$1" == "latest" ]; then
  ...
  # $OPENSHIFT_NAMESPACE_ACCELERATOR
  docker tag adorsys/$PSD2_SANDBOX_IMAGE:latest $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_ACCELERATOR/$PSD2_SANDBOX_IMAGE:latest
  docker push $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_ACCELERATOR/$PSD2_SANDBOX_IMAGE:latest
  docker tag adorsys/$SSL_PROXY_IMAGE:latest $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_ACCELERATOR/$SSL_PROXY_IMAGE:latest
  docker push $OPENSHIFT_REGISTRY/$OPENSHIFT_NAMESPACE_ACCELERATOR/$SSL_PROXY_IMAGE:latest
